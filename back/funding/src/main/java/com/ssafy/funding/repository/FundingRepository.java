...
public interface FundingRepository extends CrudRepository<Funding, Integer> {
    ...
    @Query(
            value =
                    "select movie.movie_id movieId, movie.poster, funding.people_count crowdCnt, "
                            + "ifnull((SELECT sum(count) FROM orders WHERE orders.funding_id = funding.funding_id and orders.status=true),0) AS peopleCnt, "
                            + "movie.status "
    ...
    @Query(
            value =
                    "select movie.movie_id movieId, movie.title, movie.poster, funding.date,"
                            ...
                            + "where ((orders.user_id=:userId and orders.status=true and movie.status='CLOSED' and (select funding_final_result from funding_history where funding_history.funding_id=funding.funding_id)=true)) or (orders.user_id=:userId and orders.status=true and movie.status='RESERVATION') "
    ...
    @Query(
            value =
                    "select movie.movie_id movieId, movie.title, movie.poster, funding.date,"
                            + "ifnull((select sum(count) from orders where orders.funding_id=funding.funding_id and orders.status=true),0) attendCnt,"
                            + "funding.people_count goalCnt from funding "
                            + "join movie on movie.movie_id=funding.movie_id "
                            + "join orders on orders.funding_id=funding.funding_id "
                            + "where orders.user_id=:userId and orders.status=true and movie.status='CLOSED' and (select funding_final_result from funding_history where funding_history.funding_id=funding.funding_id)=false "
