...
const ReservePage = () => {
  ...
  return (
    <div className="h-[100vh] flex flex-col">
      <NoneNavHeader centerText="좌석예매" onBackButtonClick={handleBackButtonClick} />
      {isLoading ? (
        <Loading />
      ) : (
        <>
            ...
            <div className="flex-1 flex flex-col gap-[5vh] p-[3vh]">
              ...
              <div className="flex flex-col items-center gap-[5vh]">
                <div className="relative flex flex-col items-center">
                  <img className="w-[100%] h-[5vh]" src={screen} alt="영화관스크린" />
                    ...
                    <div key={row} className="flex gap-[2vh]">
                      <div className="flex items-center justify-center gap-[1vh]">
                        {numbers.map((number) => {
                          ...
                          return <Seat key={seatId} seatId={seatId} max={maxSeats} isOccupied={occupiedSeats.includes(seatId)} />;
                        })}
                      </div>
                    </div>
                  ))}
      ...
      {isModalOpen && <Prompt onClose={closeModal} onConfirm={confirmAndGoBack} />}
      <ToasterMsg />
    </div>
  );
};
