...
import { postSurvey } from "@api/funding";
...
const HopeSurvey = ({ id, modalDown }: Props) => {
      ...
      <div className={surveyArea}>
        ...
        <div className="relative w-[90%] h-[30vh] p-3">
          <img
            ...
            src={checkArea == 1 ? redCheck : checkArea == 2 ? yellowCheck : checkArea == 3 ? greenCheck : checkArea == 4 ? blueCheck : map}
            alt=""
          />
          <svg className="absolute w-[100%] h-[100%] top-0 left-0 opacity-0" viewBox="0 0 600 470">
            <polygon
            ...
            />
            <polygon points="334,441 392,550 474,500 554,450 593,320 600,190 510,220 479,313 440,321 386,291 310,342 330,356 317,374" fill="blue" onClick={() => setCheckArea(4)} />
          </svg>
        </div>
        <div>원하는 상영 시간을 골라주세요(시작시간 기준)</div>
        <div className="grid grid-cols-2 gap-[2vh]">
          <div className={`text-center p-4 ${checkTime == 9 ? "choose" : ""}`} onClick={() => setCheckTime(9)}>
            9:00~12:00
          </div>
          <div className={`text-center p-4 ${checkTime == 12 ? "choose" : ""}`} onClick={() => setCheckTime(12)}>
            12:00~15:00
          </div>
          <div className={`text-center p-4 ${checkTime == 15 ? "choose" : ""}`} onClick={() => setCheckTime(15)}>
            15:00~18:00
          </div>
          <div className={`text-center p-4 ${checkTime == 18 ? "choose" : ""}`} onClick={() => setCheckTime(18)}>
            18:00~21:00
          </div>
        </div>
        <div className="w-[100%] text-center survey-submit px-auto py-4 border-red-700" onClick={() => sendSurvey()}>
          확인
        </div>
      </div>
    </div>
