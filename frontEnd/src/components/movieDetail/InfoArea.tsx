...
const InfoArea = ({ status, fundInfo }: Props) => {
  ...
  useEffect(() => {
    if (status === "무딩 예정" || status === "무딩 준비 중") {
      setSeeInfo(false);
    } else {
      if (fundInfo) {
        ...
        <>
          ...
          <div className="flex justify-around items-center mt-6">
            ...
            <div className="flex flex-col items-center gap-2">
              <div className={`text-[1.5vh] ${status === "예매예정" || status === "예매 진행" ? "text-red-500" : ""}`}>좌석 예매일</div>
              <div className={`${status === "예매예정" || status === "예매 진행" ? "active" : "not-active"}`}>
                {" "}
                <img className="absolute z-[2] w-[85%] top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%]" src={moding2} alt="" />
              </div>
              <div className={`text-[1.5vh] ${status === "예매예정" || status === "예매 진행" ? "text-red-500" : ""}`}>
                {bookDate && (
                  <>
